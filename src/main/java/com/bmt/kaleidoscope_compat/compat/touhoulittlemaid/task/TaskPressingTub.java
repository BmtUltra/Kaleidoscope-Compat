package com.bmt.kaleidoscope_compat.compat.touhoulittlemaid.task;

import com.bmt.kaleidoscope_compat.compat.touhoulittlemaid.MaidPressingTubBehavior;
import com.github.tartaricacid.touhoulittlemaid.api.task.IMaidTask;
import com.github.tartaricacid.touhoulittlemaid.entity.passive.EntityMaid;
import com.github.tartaricacid.touhoulittlemaid.init.InitSounds;
import com.github.tartaricacid.touhoulittlemaid.util.SoundUtil;
import com.google.common.collect.Lists;
import com.mojang.datafixers.util.Pair;
import net.minecraft.resources.ResourceLocation;
import net.minecraft.sounds.SoundEvent;
import net.minecraft.world.entity.ai.behavior.BehaviorControl;
import net.minecraft.world.item.Item;
import net.minecraft.world.item.ItemStack;
import net.minecraftforge.registries.ForgeRegistries;
import org.jetbrains.annotations.NotNull;

import java.util.List;

public class TaskPressingTub implements IMaidTask {
    public static final ResourceLocation UID = ResourceLocation.fromNamespaceAndPath("kaleidoscope_compat", "pressing_tub");

    @Override
    public @NotNull ResourceLocation getUid() {
        return UID;
    }

    @Override
    public @NotNull ItemStack getIcon() {
        Item pressingTub = ForgeRegistries.ITEMS.getValue(
                ResourceLocation.fromNamespaceAndPath("kaleidoscope_tavern", "pressing_tub")
        );
        return pressingTub.getDefaultInstance();
    }

    @Override
    public @NotNull SoundEvent getAmbientSound(@NotNull EntityMaid maid) {
        return SoundUtil.environmentSound(maid, InitSounds.MAID_IDLE.get(), 0.5F);
    }

    @Override
    public @NotNull List<Pair<Integer, BehaviorControl<? super EntityMaid>>> createBrainTasks(@NotNull EntityMaid maid) {
        return Lists.newArrayList(
                Pair.of(5, new MaidPressingTubBehavior(0.6F))
        );
    }

    @Override
    public @NotNull String getMaidActionSummary() {
        return "Press ingredients in the pressing tub from Kaleidoscope Tavern";
    }
}